...
export function calculateResults(input: RawResult[]): Result[] {
  ...
  for (const result of results) {
    for (const bm of result.benchmarks) {
      ...
      bm.duration = measurements.duration.reduce((sum, cur) => sum + cur.duration, 0) / effectiveSampleSize;
      const durvar = measurements.duration.reduce((sum, cur) => sum + Math.pow(cur.duration - bm.duration!, 2), 0) / (effectiveSampleSize - 1);
      bm.durationStdDev = Math.sqrt(durvar);
      const zScore95 = 1.96;
      const durationSEM = bm.durationStdDev / Math.sqrt(effectiveSampleSize);
      bm.durationMOE = zScore95 * durationSEM;

      bm.memory = measurements.memory.reduce((sum, cur) => sum + cur.memory, 0) / effectiveSampleSize;
      const memvar = measurements.memory.reduce((sum, cur) => sum + Math.pow(cur.memory - bm.memory!, 2), 0) / (effectiveSampleSize - 1);
      bm.memoryStdDev = Math.sqrt(memvar);
      const memorySEM = bm.memoryStdDev / Math.sqrt(effectiveSampleSize);
  ...
  for (const result of results) {
    ...
    result.benchmarks.unshift({
      name: COMPOSITE_NAME,
      measurements: [],
      normalDuration: Math.pow(totalDurNorm, 1 / result.benchmarks.length),
      normalMemory: Math.pow(totalMemNorm, 1 / result.benchmarks.length),
      duration: 0,
      memory: 0,
      durationMOE: 0,
      memoryMOE: 0
